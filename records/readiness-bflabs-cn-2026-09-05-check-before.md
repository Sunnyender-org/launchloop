...
- 档: **lite**  · 仓库: /Users/sunny/Work/CC/bflabs-agent-readiness  · 时间: 2026-09-05 21:04:18
- 结果: 10 pass · 4 fail · 4 warn · 2 unknown · 7 manual
- **硬门槛失败: 4** → 先修下面的 FAIL

## 先修这些（硬门槛 FAIL）

- **L01 http → https 跳转** — http 未跳转到 https，status 200
  - 修法: 在 CDN / 服务器上加 301 http→https
- **F02 AI 搜索爬虫放行** — 被拦: ClaudeBot, Google-Extended; GPTBot(训练用) 拦截 —— 这是独立决策，记录理由即可
  - 修法: robots.txt 为 OAI-SearchBot / PerplexityBot / ClaudeBot / Google-Extended / Bingbot 显式 Allow: /
- **F05 title / description / canonical / 单 h1 / lang** — 无 canonical
  - 修法: 用框架的 Metadata API 补齐；每页唯一 title 与 canonical
- **F07 JSON-LD 结构化数据** — types=[] invalid_blocks=0
  - 修法: 首页至少 Organization + SoftwareApplication/Product；内容需与可见文本一致；模板见 launchloop-implement/templates/jsonld.md

## 门 · Ready

| ID | 检查 | 状态 | 硬 | 证据 |
|---|---|---|---|---|
| R01 | 安全响应头 | WARN | S | 缺少: strict-transport-security, x-content-type-options, x-frame-options\|csp frame-ancestors, referrer-policy |
...
| ID | 检查 | 状态 | 硬 | 证据 |
|---|---|---|---|---|
| L01 | http → https 跳转 | FAIL | H | http 未跳转到 https，status 200 |
...
| F02 | AI 搜索爬虫放行 | FAIL | H | 被拦: ClaudeBot, Google-Extended; GPTBot(训练用) 拦截 —— 这是独立决策，记录理由即可 |
...
| F05 | title / description / canonical / 单 h1 / lang | FAIL | H | 无 canonical |
| F06 | 首页服务端渲染出可读文本（AI 爬虫不跑 JS） | PASS | H | 禁 JS 可见文本 1233 字符 |
| F07 | JSON-LD 结构化数据 | FAIL | H | types=[] invalid_blocks=0 |
| F08 | llms.txt | PASS | H | 8 个绝对链接, content-type=text/plain |
| F09 | Open Graph 分享卡 | WARN | S | 缺 og:title, og:description, og:image |
